...
def run(source,keep=False):
    ...
    try:
        ...
        if result["proof_overhead_ratio"]>.1 or result["verification_rows_per_second"]<500 or result["incremental_replicas"]!=1 or result["empty_relay_replicas"]!=initial_counts["remote.row_proofs"]: raise AssertionError("remote release gate failed")
