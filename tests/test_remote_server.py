...
import pytest
from ai_convos_remote.protocol import certificate, event, identity, seal_event, seal_key, sign_control
...
def test_personal_workspace_idempotency_and_ciphertext_only(tmp_path):
    ...
    action(db,sign_control(a["device"],{"op":"create","workspace":ws,"kind":"personal","envelope":seal_key(key,a["device"]["box_public"],f"workspace:{ws}:epoch:1")}),a["token"])
...
def test_team_add_default_history_grant_remove_and_rotation(tmp_path):
    ...
    action(db,sign_control(a["device"],{"op":"create","workspace":ws,"kind":"team","envelope":seal_key(k1,a["device"]["box_public"],f"workspace:{ws}:epoch:1")}),a["token"])
    ...
    action(db,sign_control(a["device"],{"op":"rotate","workspace":ws,"epoch":2,"members":{a["user"]:"admin",b["user"]:"member"},"envelopes":env2}),a["token"])
    ...
    action(db,sign_control(a["device"],{"op":"grant_all","workspace":ws,"user":b["user"],"envelopes":{"1":{b["device"]["id"]:old_for_b}}}),a["token"])
    ...
    action(db,sign_control(a["device"],{"op":"rotate","workspace":ws,"epoch":3,"members":{a["user"]:"admin"},"envelopes":env3}),a["token"])
...
def test_device_certificate_recovery_and_author_acl(tmp_path):
    ...
    ws,key = "personal",bytes([5])*32; action(db,sign_control(a["device"],{"op":"create","workspace":ws,"kind":"personal","envelope":seal_key(key,a["device"]["box_public"],f"workspace:{ws}:epoch:1")}),a["token"])
...
def test_large_events_are_manifested_then_fetched(tmp_path):
    db=connect(tmp_path/"server.db"); a=account(db,"alice"); ws,key="personal",bytes([7])*32; action(db,sign_control(a["device"],{"op":"create","workspace":ws,"kind":"personal","envelope":seal_key(key,a["device"]["box_public"],f"workspace:{ws}:epoch:1")}),a["token"])
...
def test_restart_preserves_events_tokens_and_idempotency(tmp_path):
    path=tmp_path/"server.db"; db=connect(path); a=account(db,"alice"); ws,key="personal",bytes([8])*32; action(db,sign_control(a["device"],{"op":"create","workspace":ws,"kind":"personal","envelope":seal_key(key,a["device"]["box_public"],f"workspace:{ws}:epoch:1")}),a["token"]); env=seal_event(event(a["device"],1,"x","x",{},[],"2026-01-01T00:00:00Z"),ws,1,key); action(db,{"op":"upload","envelope":env},a["token"]); db.close(); db=connect(path)
