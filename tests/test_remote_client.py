...
from ai_convos.cli import ARCHIVE_COLUMNS, archive_state, capture_provenance, index_attachment_body, init_schema, project_archive_row
...
def test_holder_refounds_lost_team_on_fresh_relay_without_excluded_member(tmp_path,monkeypatch):
    old=server_connect(tmp_path/"old.db"); monkeypatch.setattr("ai_convos_remote.request",transport(old)); a,b=tmp_path/"alice",tmp_path/"bob"; alice,_=setup_client("http://old","alice",root=a); setup_client("http://old","bob",root=b); origin=create(alice,"Old Team","team",a); add_member(alice,origin,"bob",root=a); alice=load(a); path=a/"data/convos.db"; write_archive(path,"portable original"); db=duckdb.connect(str(path)); db.execute("INSERT INTO messages VALUES ('m','c','user','portable child',NULL,'2026-01-01',NULL,'{}',NULL,NULL)"); db.close(); sa=connect(a/"remote/state.db"); core=duckdb.connect(str(path),read_only=True); records=scan(core,sa); core.close(); projection_module.attest_rows(path,alice,origin,records); envs=projection_module.row_replicas(path,alice,origin,records,{2:key(alice,origin,2)}); action(old,{"op":"replica_upload_many","envelopes":envs},alice["token"]); pull(load(b),connect(b/"remote/state.db"),b)
    ...
    db=duckdb.connect(str(tmp_path/"carol/data/convos.db"),read_only=True); assert db.execute("SELECT title FROM conversations").fetchone()[0]=="portable original" and db.execute("SELECT content FROM messages").fetchone()[0]=="portable child" and set(db.execute("SELECT workspace_id,authorization_workspace_id,author_user_id FROM remote.row_proofs").fetchall())=={(origin,origin,alice["user"])}; db.close(); assert fresh.execute("SELECT COUNT(*) FROM origin_bundles").fetchone()[0]==1 and fresh.execute("SELECT COUNT(*) FROM row_replicas").fetchone()[0]==2
...
def test_deleted_state_rebaselines_before_publishing_existing_archive(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr("ai_convos_remote.request",direct); sync_once(b,True); assert server.execute("SELECT COUNT(*) FROM events").fetchone()[0]==before and server.execute("SELECT COUNT(*) FROM row_replicas").fetchone()[0]==replicas_before+1
