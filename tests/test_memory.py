...
def test_remote_memory_replays_updates_conflicts_and_tombstones_without_paths(tmp_path, monkeypatch):
    ...
    acknowledged(state,three,cursor=3); memory_module.forget_data(created["id"],str(repo_a)); deleted = remote_records(a,state,"personal","personal")[0]
...
def test_remote_tombstone_redacts_origin_but_preserves_mixed_local_memory(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(b)); linked=memory_module.remember_data("shared local truth","global"); assert linked["status"]=="linked"; monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); memory_module.forget_data(created["id"],"global"); acknowledged(state,active); deleted=remote_records(a,state,"personal","personal")[0]; tombstone=event(device,2,deleted["kind"],deleted["entity"],deleted["payload"],[active["id"]],observed_at="2026-01-02T00:00:00Z"); assert remote_project(b,state,tombstone,"personal","b")
