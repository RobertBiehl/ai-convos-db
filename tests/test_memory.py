...
def test_schema_v1_migration_indexes_existing_repo_scope_for_new_clone(tmp_path, monkeypatch):
    ...
    db = sqlite3.connect(tmp_path/"memory.db"); assert db.execute("PRAGMA user_version").fetchone()[0] == 5 and db.execute("SELECT COUNT(DISTINCT scope),COUNT(DISTINCT checkout) FROM repository_scopes").fetchone() == (1,2); db.close()
...
def test_ledger_adopts_version_zero_and_rejects_newer_schema(tmp_path, monkeypatch):
    ...
    scan_store(); db = sqlite3.connect(path); assert db.execute("PRAGMA user_version").fetchone()[0] == 5 and db.execute("SELECT name FROM sqlite_master WHERE name='legacy'").fetchone()
    db.execute("DROP TABLE evidence"); db.execute("PRAGMA user_version=4"); db.commit(); db.close(); scan_store(); db=sqlite3.connect(path); assert db.execute("PRAGMA user_version").fetchone()[0]==5 and db.execute("SELECT name FROM sqlite_master WHERE name='evidence'").fetchone()==("evidence",)
    db.execute("DROP TABLE projections"); db.execute("PRAGMA user_version=6"); db.commit(); db.close()
    with pytest.raises(ValueError, match="ledger schema version"): scan_store()
    db = sqlite3.connect(path); assert db.execute("PRAGMA user_version").fetchone()[0] == 6 and not db.execute("SELECT name FROM sqlite_master WHERE name='projections'").fetchone(); db.close()
...
def test_user_owned_commands_refuse_provider_backed_canonical(tmp_path, monkeypatch):
    ...
    assert revise.exit_code == forget.exit_code == 1 and "without provider origins" in revise.output and "without provider origins" in forget.output
...
def test_remember_attaches_local_archive_evidence_to_exact_revisions(tmp_path, monkeypatch):
    from ai_convos_remote.projection import connect as remote_connect
    ...
    target=Path(backup_data(tmp_path/"with-evidence.db")["path"]); remote=tmp_path/"remote-root"; (remote/"memory").mkdir(parents=True); (remote/"memory"/"state.db").write_bytes(target.read_bytes()); state=remote_connect(tmp_path/"remote-state.db"); payload=json.dumps(remote_records(remote,state,"personal","personal")); state.close()
...
def test_restore_migrates_valid_older_snapshot_without_modifying_source(tmp_path, monkeypatch):
    roots(tmp_path,monkeypatch); created=memory_module.remember_data("before schema upgrade","/repo"); current=Path(backup_data(tmp_path/"current.db")["path"]); legacy=tmp_path/"schema3.db"; legacy.write_bytes(current.read_bytes()); db=sqlite3.connect(legacy); db.execute("DROP TABLE remote_parts"); db.execute("PRAGMA user_version=3"); db.commit(); db.close(); memory_module.remember_data("new live state","/repo")
    preview=restore_data(legacy); assert preview["status"]=="would_restore" and preview["canonicals"]==1 and sqlite3.connect(legacy).execute("PRAGMA user_version").fetchone()[0]==3 and not sqlite3.connect(legacy).execute("SELECT name FROM sqlite_master WHERE name='remote_parts'").fetchone()
    restored=restore_data(legacy,True); db=sqlite3.connect(tmp_path/"memory.db"); assert restored["status"]=="restored" and db.execute("PRAGMA user_version").fetchone()[0]==5 and db.execute("SELECT id,content FROM canonicals").fetchall()==[(created["id"],"before schema upgrade")] and db.execute("SELECT name FROM sqlite_master WHERE name='remote_parts'").fetchone()==("remote_parts",) and db.execute("SELECT COUNT(*) FROM evidence").fetchone()[0]==0; db.close()
...
def test_remote_memory_replays_updates_conflicts_and_tombstones_without_paths(tmp_path, monkeypatch):
    from ai_convos_remote.projection import connect as remote_connect
    from ai_convos_remote.protocol import digest, event, identity
    ...
    monkeypatch.delenv("CONVOS_MEMORY_DB"); monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); created = memory_module.remember_data("portable one",str(repo_a)); state = remote_connect(tmp_path/"remote.db"); record = remote_records(a,state,"personal","personal")[0]
    assert remote_records(a,state,"team","team") == [] and str(repo_a) not in json.dumps(record) and record["payload"]["repository"].startswith("repo_")
    device = identity("a"); one = event(device,1,record["kind"],record["entity"],record["payload"],observed_at="2026-01-01T00:00:00Z"); assert remote_project(b,state,one,"personal","b")
    ...
    monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); memory_module.remember_data("portable two",str(repo_a),created["id"]); record = remote_records(a,state,"personal","personal")[0]; two = event(device,2,record["kind"],record["entity"],record["payload"],[one["id"]],observed_at="2026-01-02T00:00:00Z"); assert remote_project(b,state,two,"personal","b") and not remote_project(b,state,one,"personal","b")
    db = memory_module.connect(b); cid = db.execute("SELECT id FROM canonicals WHERE scope=?",(target,)).fetchone()[0]; assert db.execute("SELECT content FROM canonicals WHERE id=?",(cid,)).fetchone()[0] == "portable two"; memory_module._canonical(db,cid,target,"local divergence","2026-01-02T12:00:00Z"); db.commit(); db.close()
    monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); memory_module.remember_data("portable three",str(repo_a),created["id"]); record = remote_records(a,state,"personal","personal")[0]; three = event(device,3,record["kind"],record["entity"],record["payload"],[two["id"]],observed_at="2026-01-03T00:00:00Z"); assert remote_project(b,state,three,"personal","b")
    ...
    replay = event(device,3,record["kind"],record["entity"],{**record["payload"],"content":__import__("base64").b64encode(b"replay").decode(),"hash":memory_module._hash("replay")},[two["id"]],observed_at="2026-01-03T00:00:01Z")
    with pytest.raises(ValueError,match="sequence replay"): remote_project(b,state,replay,"personal","b")
    state.execute("INSERT INTO event_log VALUES (?,?,?,?,?,?)",("personal",three["id"],3,"out",json.dumps(three),"{}")); state.execute("INSERT INTO published VALUES (?,?,?,?)",("personal",record["entity"],digest(record["payload"]),three["id"])); state.commit(); memory_module.forget_data(created["id"],str(repo_a)); deleted = remote_records(a,state,"personal","personal")[0]
    assert deleted["payload"]["status"] == "deleted" and "content" not in deleted["payload"]; tombstone = event(device,4,deleted["kind"],deleted["entity"],deleted["payload"],[three["id"]],observed_at="2026-01-04T00:00:00Z"); assert remote_project(b,state,tombstone,"personal","b")
    db = memory_module.connect(b); assert tuple(db.execute("SELECT active,content FROM sources WHERE provider='remote'").fetchone()) == (0,"") and db.execute("SELECT COUNT(*) FROM revisions r JOIN sources s ON s.id=r.source WHERE s.provider='remote'").fetchone()[0] == 0 and db.execute("SELECT content FROM canonicals WHERE id=?",(cid,)).fetchone()[0] == "local divergence" and plan_data(db=db,scope=target)["pending"][0]["kind"] == "missing"; db.close(); state.close()


def test_large_remote_memory_chunks_reassemble_out_of_order_without_lazy_payloads(tmp_path, monkeypatch):
    from ai_convos_remote.projection import connect as remote_connect
    from ai_convos_remote.protocol import digest, event, identity
    roots(tmp_path,monkeypatch); a,b=tmp_path/"a",tmp_path/"b"; a.mkdir(); monkeypatch.delenv("CONVOS_MEMORY_DB"); monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); first="large marker\n"+"x"*70000; created=memory_module.remember_data(first,"global"); state=remote_connect(tmp_path/"remote.db"); records=remote_records(a,state,"personal","personal")
    assert len(records)==3 and all(len(r["payload"]["content"])<=32768 and "large marker" not in json.dumps(r) for r in records)
    device=identity("a"); events=[]; parent=[]
    for seq,record in enumerate(records,1): events.append(value:=event(device,seq,record["kind"],record["entity"],record["payload"],parent,observed_at=f"2026-01-01T00:00:0{seq}Z")); parent=[value["id"]]
    assert remote_project(b,state,events[2],"personal","b") and remote_project(b,state,events[0],"personal","b"); db=memory_module.connect(b); assert db.execute("SELECT COUNT(*) FROM canonicals").fetchone()[0]==0 and db.execute("SELECT COUNT(*) FROM remote_parts").fetchone()[0]==2; db.close()
    assert remote_project(b,state,events[1],"personal","b"); db=memory_module.connect(b); assert db.execute("SELECT content FROM canonicals").fetchone()[0]==first and db.execute("SELECT COUNT(*) FROM remote_parts").fetchone()[0]==0; db.close()
    monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); second="new marker\n"+"y"*70000; memory_module.remember_data(second,"global",created["id"]); records=remote_records(a,state,"personal","personal"); newer=[]
    for seq,record in enumerate(records,4): newer.append(value:=event(device,seq,record["kind"],record["entity"],record["payload"],parent,observed_at=f"2026-01-02T00:00:0{seq}Z")); parent=[value["id"]]
    assert remote_project(b,state,newer[2],"personal","b") and remote_project(b,state,newer[0],"personal","b") and not remote_project(b,state,events[0],"personal","b"); db=memory_module.connect(b); assert db.execute("SELECT content FROM canonicals").fetchone()[0]==first; db.close()
    assert remote_project(b,state,newer[1],"personal","b"); db=memory_module.connect(b); assert db.execute("SELECT content FROM canonicals").fetchone()[0]==second and db.execute("SELECT COUNT(*) FROM remote_parts").fetchone()[0]==0; db.close(); state.close()


def test_remote_tombstone_purges_unchanged_remote_only_memory_and_decrypted_events(tmp_path, monkeypatch):
    from ai_convos_remote.projection import connect as remote_connect
    from ai_convos_remote.protocol import digest, event, identity
    ...
    outgoing.execute("INSERT INTO event_log VALUES (?,?,?,?,?,?)",("personal",active["id"],1,"out",json.dumps(active),"{}")); outgoing.execute("INSERT INTO published VALUES (?,?,?,?)",("personal",record["entity"],digest(record["payload"]),active["id"])); outgoing.commit(); incoming.execute("INSERT INTO event_log VALUES (?,?,?,?,?,NULL)",("personal",active["id"],1,"in",json.dumps(active))); assert remote_project(b,incoming,active,"personal","b"); incoming.commit()
    memory_module.forget_data(created["id"],"global"); deleted=remote_records(a,outgoing,"personal","personal")[0]; tombstone=event(device,2,deleted["kind"],deleted["entity"],deleted["payload"],[active["id"]],observed_at="2026-01-02T00:00:00Z"); incoming.execute("INSERT INTO event_log VALUES (?,?,?,?,?,NULL)",("personal",tombstone["id"],2,"in",json.dumps(tombstone))); assert remote_project(b,incoming,tombstone,"personal","b"); incoming.rollback()
    incoming.execute("INSERT INTO event_log VALUES (?,?,?,?,?,NULL)",("personal",tombstone["id"],2,"in",json.dumps(tombstone))); assert not remote_project(b,incoming,tombstone,"personal","b"); incoming.commit()
    outgoing.execute("INSERT INTO event_log VALUES (?,?,?,?,?,?)",("personal",tombstone["id"],2,"out",json.dumps(tombstone),"{}")); outgoing.execute("INSERT INTO published VALUES (?,?,?,?)",("personal",deleted["entity"],digest(deleted["payload"]),tombstone["id"])); outgoing.commit(); assert memory_module.remote_purges(a,outgoing,"personal","personal")==[active["id"]]
    db=memory_module.connect(b); assert db.execute("SELECT COUNT(*) FROM canonicals").fetchone()[0]==db.execute("SELECT COUNT(*) FROM sources").fetchone()[0]==db.execute("SELECT COUNT(*) FROM revisions").fetchone()[0]==0; db.close(); rows=incoming.execute("SELECT event_json FROM event_log WHERE json_extract(event_json,'$.kind')='memory.canonical'").fetchall(); assert len(rows)==1 and "forget me remotely" not in rows[0][0]; outgoing.close(); incoming.close()


def test_remote_tombstone_redacts_origin_but_preserves_mixed_local_memory(tmp_path, monkeypatch):
    from ai_convos_remote.projection import connect as remote_connect
    from ai_convos_remote.protocol import digest, event, identity
    roots(tmp_path,monkeypatch); a,b=tmp_path/"a",tmp_path/"b"; a.mkdir(); monkeypatch.delenv("CONVOS_MEMORY_DB"); monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); created=memory_module.remember_data("shared local truth","global"); state=remote_connect(tmp_path/"remote.db"); record=remote_records(a,state,"personal","personal")[0]; device=identity("a"); active=event(device,1,record["kind"],record["entity"],record["payload"],observed_at="2026-01-01T00:00:00Z"); assert remote_project(b,state,active,"personal","b")
    monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(b)); linked=memory_module.remember_data("shared local truth","global"); assert linked["status"]=="linked"; monkeypatch.setenv("CONVOS_PROJECT_ROOT",str(a)); memory_module.forget_data(created["id"],"global"); state.execute("INSERT INTO event_log VALUES (?,?,?,?,?,?)",("personal",active["id"],1,"out",json.dumps(active),"{}")); state.execute("INSERT INTO published VALUES (?,?,?,?)",("personal",record["entity"],digest(record["payload"]),active["id"])); state.commit(); deleted=remote_records(a,state,"personal","personal")[0]; tombstone=event(device,2,deleted["kind"],deleted["entity"],deleted["payload"],[active["id"]],observed_at="2026-01-02T00:00:00Z"); assert remote_project(b,state,tombstone,"personal","b")
    db=memory_module.connect(b); sources=[tuple(r) for r in db.execute("SELECT provider,active,content FROM sources ORDER BY provider")]; assert db.execute("SELECT content FROM canonicals").fetchone()[0]=="shared local truth" and sources==[("remote",0,""),("user",1,"shared local truth")] and plan_data(db=db,scope="global")["pending"][0]["kind"]=="missing"; db.close(); state.close()
