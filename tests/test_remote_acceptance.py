...
def test_real_http_background_two_device_delivery_under_ten_seconds(tmp_path):
    ...
    try:
        wait(url+"/v3/health"); a,b=tmp_path/"a",tmp_path/"b"; _,recovery=setup_client(url,"alice","laptop",root=a); setup_client(url,"alice","desktop",recovery,root=b)
...
def test_real_http_background_delivers_large_encrypted_memory_without_lazy_fetch(tmp_path):
    ...
    try:
        ...
        while time.time()<end:
            try: db=sqlite3.connect(b/"memory/state.db"); forgotten=db.execute("SELECT COUNT(*) FROM canonicals").fetchone()[0]==0; db.close(); relay=sqlite3.connect(tmp_path/"server.db"); forgotten=forgotten and relay.execute(f"SELECT COUNT(*) FROM event_purges WHERE event IN ({','.join('?'*len(old))})",old).fetchone()[0]==len(old); relay.close()
...
def test_real_http_team_policy_across_different_checkout_paths(tmp_path):
    ...
    try:
        ...
        while time.time()<end:
            ...
            if found: break
            time.sleep(.2)
        assert found; check=duckdb.connect(str(b/"data/convos.db"),read_only=True); activity=query(check,"repository_activity",rid); check.close(); assert activity and activity[0]["repository"]==rid
