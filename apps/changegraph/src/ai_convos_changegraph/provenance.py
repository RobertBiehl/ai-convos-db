...
_HISTORY=f"""SELECT fe.created_at observed_at,f.repository,f.path,fe.edit_type,x.evidence,fe.message_id changeset_id,COALESCE(m.conversation_id,'unknown') conversation,fe.message_id turn,{_PROMPT} prompt,x.before_hash,x.after_hash FROM provenance.file_edit_files x JOIN provenance.files f ON f.id=x.file_id JOIN file_edits fe ON fe.id=x.file_edit_id LEFT JOIN messages m ON m.id=fe.message_id"""
_QUERIES={
    "file_history":(_HISTORY,"path"),
    "changeset_files":(f"SELECT fe.message_id id,m.conversation_id,fe.message_id turn,{_PROMPT} prompt,f.repository,f.path,fe.edit_type,x.evidence FROM provenance.file_edit_files x JOIN provenance.files f ON f.id=x.file_id JOIN file_edits fe ON fe.id=x.file_edit_id JOIN messages m ON m.id=fe.message_id","id"),
    ...
    "commit_conversations":("SELECT p.head,m.conversation_id conversation,fe.message_id changeset_id,p.repository,x.evidence FROM provenance.checkpoint_edits x JOIN provenance.git_checkpoints p ON p.id=x.checkpoint_id JOIN file_edits fe ON fe.id=x.file_edit_id JOIN messages m ON m.id=fe.message_id","head"),
    ...
    "identity_assertions":("SELECT * FROM provenance.assertions","left_id"),
    "capture_gaps":("SELECT * FROM provenance.capture_gaps","repository"),
    "checkpoint_states":("SELECT repository,head,state_hash,paths,observed_at FROM provenance.git_checkpoints","repository"),
