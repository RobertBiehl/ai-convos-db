...
def verify_state(value,previous=None):
    ...
    if action in ("self_approve","quorum_approve","personal_recover"):
        ...
        if set(value["devices"])!=set(previous["devices"]) or any({k:v for k,v in d.items() if k!="history"}!={k:v for k,v in previous["devices"][i].items() if k!="history"} or previous["devices"][i]["history"] and not d["history"] for i,d in value["devices"].items()) or value["removed"]!=previous["removed"] or value["epoch"]!=previous["epoch"] or value["key_commitment"]!=previous["key_commitment"] or set(value["members"])!=set(previous["members"]) or any(not _same(m,previous["members"][u],"role","joined") or not set(previous["members"][u]["selected"])<=set(m["selected"]) for u,m in value["members"].items()) or not admin: raise ValueError("invalid history transition")
