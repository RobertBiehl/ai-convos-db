...
def edit_hooks(remove=False,root=None):
    wake=Path(root or os.environ.get("CONVOS_PROJECT_ROOT",Path.home()/".convos"))/"remote/wake"; cmd=f"mkdir -p {shlex.quote(str(wake.parent))} && touch {shlex.quote(str(wake))} # ai-convos remote hook"; configs=((Path(os.environ.get("CLAUDE_CONFIG_DIR",Path.home()/".claude"))/"settings.json",("Stop","SessionEnd")),(Path(os.environ.get("CODEX_HOME",Path.home()/".codex"))/"hooks.json",("Stop",)))
...
def enable(data_dir,remove=False):
    data_dir=Path(data_dir).resolve(); cmd=edit_hooks(remove,data_dir.parent); data_dir.mkdir(parents=True,exist_ok=True); root=str(data_dir.parent); label="com.ai-convos.remote"
    ...
    plist.parent.mkdir(parents=True,exist_ok=True); plist.write_bytes(plistlib.dumps({"Label":label,"ProgramArguments":[shutil.which("convos") or "convos","remote","watch","--interval","2"],"EnvironmentVariables":{"CONVOS_PROJECT_ROOT":root},"KeepAlive":True,"RunAtLoad":True,"StandardOutPath":str(data_dir/"worker.log"),"StandardErrorPath":str(data_dir/"worker.log")})); subprocess.run(("launchctl","bootout",f"gui/{os.getuid()}/{label}"),capture_output=True); subprocess.run(("launchctl","bootstrap",f"gui/{os.getuid()}",str(plist)),check=True); return f"Remote background sync enabled; hooks run `{cmd}`"
