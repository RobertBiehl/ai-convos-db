...
convos map MESSAGE_ID --no-open --output /private/map.html   # private clickable trail for human review
convos ask "question" -f json                                # local synthesis with exact cited turns
convos ask "question" --remember -f json                     # explicitly persist only a validated scoped answer
...
convos pulse -d 7 -f json                                     # exact cross-project activity metadata
convos eval private-retrieval.jsonl --mode both -f json        # offline exact-ID retrieval benchmark
convos library --no-open                                       # private human search/read browser
...
- Use `map` only when the user asks for a visual artifact or a trail genuinely benefits from human graph review. It creates a private local HTML file containing bounded archive snippets; report the path and retention implication, and use `--no-open` unless the user asked to open it.
- Use `ask` when the user wants a synthesized answer rather than discovery results. It runs a local model only, emits exact conversation/message citations, and fails closed to labeled evidence when citation validation fails. If its pinned models are absent, report `convos ask --setup`; never run that explicit multi-gigabyte download unless the user requested setup or authorized the download. Use `--evidence-only` when synthesis is unnecessary. Add `--remember` only when the user explicitly asks to persist the resulting answer; it scopes retrieval to `--cwd` or the current project, requires Memory before inference, and writes only after citation validation.
...
- Use `pulse` when the user asks what AI-assisted activity happened recently across projects. It reports exact archive metadata and groups existing cwd paths by their live Git root; it never includes message excerpts or infers completion, success, productivity, or unanswered work. It omits sessions below two current messages unless they captured an edit or tool call, reports that exact omitted count, and accepts `--min-messages 1` for a complete unclassified view. Cwd-less web conversations require explicit `--include-web`. Use JSON for agents. Use HTML only for a requested human artifact, treat the mode-`0600` file as private because it contains titles, paths, and IDs, and report its retention implication.
...
- Use `eval` only when the user asks to measure, compare, or regression-test retrieval quality. Its JSONL cases contain private user-owned relevance judgments; do not invent ground truth or persist a suite without the user's supplied IDs. Hybrid evaluation is local-only and will not download a missing model. Reports contain IDs and scores, not archive text.
- Use `library` only when the user asks to browse the archive as a human. It starts a long-running read-only loopback server and prints a fresh private URL containing bounded archive excerpts, paths, and IDs. Use `--no-open` unless the user asked to open it, report that the process remains active until Ctrl-C, and never share or externally expose its tokenized URL. Hybrid mode uses only cached local embeddings and never downloads a missing model.
